...
(status, results) = utility_grids.utility_download_formatted_grid_swap_rates(connection, ['USD', 'GBP', 'CHF', 'EUR', 'JPY'],settings.grid_swap_tenors, settings.grid_folder)
if not status:
    # Do Failure
    utility_email.send_email("operations@treasuryquants.com","web cron error",utility_common.dict_to_string(results) )
else:
    # Do success
    print (status,results)

(status, results) = utility_grids.utility_download_formatted_grid_fx(['USD', 'GBP', 'CHF', 'EUR', 'JPY', 'AUD', 'SGD', 'NZD'],"USD", settings.grid_folder)
if not status:
    # Do Failure
    utility_email.send_email("operations@treasuryquants.com","web cron error",utility_common.dict_to_string(results) )
else:
